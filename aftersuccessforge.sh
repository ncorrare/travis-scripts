...
if [ "$PUPPET_GEM_VERSION" = "~> 4.3" ]; then
	...
	MERGE = $(curl -vv -X PUT --data "{\"commit_message\":\"$MESSAGE\",\"sha\":\"$SHA\"}" https://$GITHUB_SECRET_TOKEN@api.github.com/repos/$TRAVIS_REPO_SLUG/pulls/$TRAVIS_PULL_REQUEST/merge | jq -r .merged)
	if [ $MERGE = true ]; then
